...
const URL     = arg('url', 'http://localhost:8806/');
const SO      = arg('secao', '');
...
const SECOES = ['topo', 'faixa', 'problema', 'servicos', 'antes-depois', 'como-funciona',
                'videos', 'clientes', 'protecao', 'faq', 'atendimento', 'cta-final'];
...
await new Promise(r => setTimeout(r, 4200)); // preloader + entrada
...
for (const id of alvos) {
  ...
  const m = await pg.evaluate(() => ({
    ...
    invisiveis: [...document.querySelectorAll('.reveal,[data-reveal],[data-split],.heroi__lead,.heroi__acoes,.heroi__palco')].filter(e => {
